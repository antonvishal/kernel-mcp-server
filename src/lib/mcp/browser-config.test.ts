...
test("browser update config forwards a start URL", () => {
  expect(
    buildBrowserUpdateConfig({ start_url: "https://example.com/checkout" }),
  ).toEqual({
    ok: true,
    value: { start_url: "https://example.com/checkout" },
  });
});
